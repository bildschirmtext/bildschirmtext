...
class Message:
    ...
    def __init__(self, dict, index):
        self.dict = dict
        self.index = index
        self.from_user = User.get(self.dict["from_user_id"], self.dict["from_ext"],
                                  self.dict.get("personal_data", False))
...
class Messaging_UI:
    ...
    def messaging_create_message_detail(user, index, is_read):
        ...
        data_cept.extend(Cept.from_str(message.from_user.user_id.ljust(12)) + b' ' + Cept.from_str(
            message.from_user.ext.rjust(5, '0')))
        ...
        data_cept.extend(
            Cept.from_str(message.from_user.first_name) + b' ' + Cept.from_str(message.from_user.last_name))
        data_cept.extend(b'\r\n')
        data_cept.extend(Cept.repeat(" ", 4))
